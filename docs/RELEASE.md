...
每周快照会在**每周一 UTC 时间 00:00**自动执行，生成 `{年份后两位}w{周数}{字母}`格式的预发布版本。

示例：
- 2026年第19周第一个快照：`26w19a`
- 2026年第19周第二个快照：`26w19b`

### 2. 手动触发

1. 访问你的 GitHub 仓库页面
2. 点击顶部的 **Actions** 标签页
3. 在左侧选择 **Weekly Snapshot Release** 工作流
4. 点击右侧的 **Run workflow** 按钮
5. （可选）输入 **base_tag** 参数：基础标签，默认为最近的正式 release
6. 点击 **Run workflow** 开始执行
...
- ✅ 按字母表顺序递增（a, b, c...）
- ✅ 标记为 Pre-release（预发布）
- ✅ 不会更新 package.json 版本号
- ✅ 包含从最近正式 release 以来的所有变更
